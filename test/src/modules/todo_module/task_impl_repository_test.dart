...
void main() {
  ...
  final MockWebSocketChannel mockWebSocketChannel = MockWebSocketChannel();
  setUpAll(() {
    dotenv.testLoad();
  });
  ...
  group('listenToGet method', () {
    test('Should have listenToGet method', () async {
      when(
        mockApiDataSource.initConnection(
          url: argThat(
            isA<String>(),
            named: 'url',
          ),
      ...
      final WebSocketChannel expectValue = expectTaskImplRepository.listenToGet(
        url: 'expectUrl',
      );
    ...
    test('Should have connect method - Failure case', () async {
      when(
        mockApiDataSource.initConnection(
          url: argThat(isA<String>(), named: 'url'),
        ),
      ...
      expect(
        () async => expectTaskImplRepository.listenToGet(
          url: 'expectUrl',
        ),
        throwsA(isA<RepositoryError>()),
      );
  ...
  group('sendData method', () {
    test('Should have sendData method', () async {
      when(
        mockApiDataSource.send(
      ...
      expectTaskImplRepository.sendData(
    ...
    test('Should have sendData method - Failure case', () async {
      when(
        mockApiDataSource.send(
          channel: argThat(isA<WebSocketChannel>(), named: 'channel'),
          data: argThat(isA<dynamic>(), named: 'data'),
      ...
      expect(
        () async => expectTaskImplRepository.sendData(
          channel: MockWebSocketChannel(),
          data: 'expectData',
        ),
        throwsA(isA<RepositoryError>()),
  ...
  group('disconnect method', () {
    test('Should have disconnect method', () async {
      ...
      expectTaskImplRepository.disconnect(
        channel: MockWebSocketChannel(),
      );
    ...
    test('Should have disconnect method - Failure case', () async {
      when(
        mockApiDataSource.close(
          channel: argThat(isA<WebSocketChannel>(), named: 'channel'),
        ),
      ...
      expect(
        () async => expectTaskImplRepository.disconnect(
          channel: MockWebSocketChannel(),
        ),
        throwsA(isA<RepositoryError>()),
      );
